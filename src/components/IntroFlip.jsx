...
export default function IntroFlip() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("intro-seen")) { setDismissed(true); return; }
    const t1 = setTimeout(() => setPhase("blink"),  900);
    const t2 = setTimeout(() => setPhase("settle"), 1500);
    const t3 = setTimeout(() => setPhase("wave"),   1900);
    const t4 = setTimeout(() => setPhase("gone"),   3800);
    const t5 = setTimeout(() => { setDismissed(true); sessionStorage.setItem("intro-seen","1"); }, 4400);
    return () => [t1,t2,t3,t4,t5].forEach(clearTimeout);
  }, []);

  const skip = () => {
    setPhase("gone");
    setTimeout(() => { setDismissed(true); sessionStorage.setItem("intro-seen","1"); }, 500);
  };
  ...
  return (
    <div className={`intro-overlay ${phase==="gone"?"intro-overlay-out":""}`}>
      ...
      <div className={`intro-avatar-free intro-phase-${phase}`}>
        ...
        {/* Speech bubble ABOVE avatar — no cutoff */}
        {(phase === "wave") && (
          <div className="intro-bubble intro-bubble-above">
            <span>Hi there! 👋</span>
            <span className="intro-bubble-sub">Welcome to my world</span>
            <div className="intro-bubble-tail-down" />
          </div>
        )}

        <svg viewBox="0 0 200 220" fill="none" xmlns="http://www.w3.org/2000/svg" className="intro-avatar-svg">
          <circle cx="100" cy="100" r="88" fill="rgba(20,18,10,0.7)" stroke="#c9a84c" strokeWidth="1.2" opacity="0.5"/>
          ...
          <ellipse cx="84" cy="108" rx="7" ry={phase==="blink"?0.5:7.5} fill="white"/>
          <ellipse cx="116" cy="108" rx="7" ry={phase==="blink"?0.5:7.5} fill="white"/>
          ...
          <g style={{ transformOrigin:"155px 175px", animation: phase==="wave" ? "waveHand 0.45s ease-in-out 0s 6 alternate" : "none" }}>
